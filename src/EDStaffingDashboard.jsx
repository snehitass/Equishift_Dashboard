...
const EDStaffingDashboard = () => {
  ...
  const calculateImpact = (baselineMetrics, recommendedChange, actualAdjustment) => {
    ...
    const recAPPsRaw = parseFloat(recommendedChange.apps) || 0;
    const recNursesRaw = parseFloat(recommendedChange.nurses) || 0;
    const isDecrease = recDoctorsRaw < 0 || recAPPsRaw < 0 || recNursesRaw < 0;

    // For calculations, use absolute values
    const recDoctors = Math.abs(recDoctorsRaw);
    const recAPPs = Math.abs(recAPPsRaw);
    ...
    const actualAPPs = actualAdjustment?.apps ?? recAPPsRaw;
    ...
    if (isDecrease) {
      ...
      const appFulfillment = recAPPs === 0 ? 1 : Math.abs(actualAPPs) / recAPPs;
      const nurseFulfillment = recNurses === 0 ? 1 : Math.abs(actualNurses) / recNurses;
      const avgFulfillment = (doctorFulfillment + appFulfillment + nurseFulfillment) / 3;
    ...
    // When rec is 0, any additions are overstaffing (1 + actual/2 so that each
    // extra person pushes into the diminishing-returns / coordination-penalty zone).
    const doctorFulfillment = recDoctors === 0 ? 1 + actualDoctors / 2 : actualDoctors / recDoctors;
    const appFulfillment = recAPPs === 0 ? 1 + actualAPPs / 2 : actualAPPs / recAPPs;
    const nurseFulfillment = recNurses === 0 ? 1 + actualNurses / 2 : actualNurses / recNurses;
    const avgFulfillment = (doctorFulfillment + appFulfillment + nurseFulfillment) / 3;
  ...
  const calculateROI = (predictedPatients, currentLwobs, baselineLwobs, doctorsAdded, appsAdded, nursesAdded) => {
  ...
  // Average census model
  const avgLengthOfStay = 3.5; // hours
  const dailyVolume = 200; // patients per day
  const avgConcurrentCensus = Math.round((dailyVolume * avgLengthOfStay) / 24); // = 29 patients

  // Ratios based on average census
  const doctorToConcurrentRatio = 10;  // 1 MD per 10 concurrent patients
  const appToConcurrentRatio = 8;      // 1 APP per 8 concurrent patients
  const nurseToConcurrentRatio = 3;    // 1 RN per 3 concurrent patients

  // Baseline staffing
  const baselineDoctors = Math.ceil(avgConcurrentCensus / doctorToConcurrentRatio);  // 29/10 = 3
  const baselineAPPs = Math.ceil(avgConcurrentCensus / appToConcurrentRatio);        // 29/8 = 4
  const baselineRNs = Math.ceil(avgConcurrentCensus / nurseToConcurrentRatio);       // 29/3 = 10

  // Day shift (60% of daily volume)
  const dayShiftVolume = dailyVolume * 0.6; // = 120 patients
  const dayShiftConcurrent = Math.round((dayShiftVolume * avgLengthOfStay) / 12); // = 35 patients
  const dayShiftDoctors = Math.ceil(dayShiftConcurrent / doctorToConcurrentRatio); // = 4
  const dayShiftAPPs = Math.ceil(dayShiftConcurrent / appToConcurrentRatio);       // = 4
  const dayShiftRNs = Math.ceil(dayShiftConcurrent / nurseToConcurrentRatio);      // = 12

  // Night shift (40% of daily volume)
  const nightShiftVolume = dailyVolume * 0.4; // = 80 patients
  const nightShiftConcurrent = Math.round((nightShiftVolume * avgLengthOfStay) / 12); // = 23 patients
  const nightShiftDoctors = Math.ceil(nightShiftConcurrent / doctorToConcurrentRatio); // = 2
  const nightShiftAPPs = Math.ceil(nightShiftConcurrent / appToConcurrentRatio);       // = 3
  const nightShiftRNs = Math.ceil(nightShiftConcurrent / nurseToConcurrentRatio);      // = 8
  ...
  const anomalyDays = [
    {
      ...
      type: 'increase',
      acuity: 'mixed',
      concurrentCensus: Math.round((240 * avgLengthOfStay) / 24),
      shifts: [
        {
          name: 'Day Shift',
          ...
          nurses: '+2',
          needsChange: true,
          reasoning: 'Mixed acuity with 35 concurrent patients (vs 29 baseline). 1 additional physician for complex cases, 1 APP for routine cases, 2 RNs to maintain 1:3 ratio.',
          baselineMetrics: { waitTime: 18, lwobs: 2.1, lengthOfStay: 3.4, census: 35 }
        },
        {
          name: 'Night Shift',
          ...
          nurses: '+2',
          needsChange: true,
          reasoning: '28 concurrent patients (vs 23 baseline) driven by end-of-week surge. Requires 1 additional physician, 1 APP, and 2 RNs to maintain safe ratios through the evening.',
          baselineMetrics: { waitTime: 14, lwobs: 1.7, lengthOfStay: 3.0, census: 28 }
        }
      ]
    },
    {
      ...
      type: 'increase',
      acuity: 'low',
      concurrentCensus: Math.round((230 * avgLengthOfStay) / 24),
      shifts: [
        {
          name: 'Day Shift',
          time: '7:00 AM - 7:00 PM',
          ...
          nurses: '+1',
          needsChange: true,
          reasoning: '34 concurrent patients (vs 29 baseline). Primarily low-acuity chest pain and minor injuries. 1 APP efficiently handles routine workups, 1 RN maintains 1:3 ratio. Physicians maintain baseline.',
          baselineMetrics: { waitTime: 22, lwobs: 3.2, lengthOfStay: 3.8, census: 34 }
        },
        {
          name: 'Night Shift',
          time: '7:00 PM - 7:00 AM',
          ...
          nurses: '+1',
          needsChange: true,
          reasoning: '27 concurrent patients (vs 23 baseline). Primarily low-acuity Valentine\'s cases carry into the evening. 1 additional physician and APP needed to maintain ratios, 1 RN for bedside care.',
          baselineMetrics: { waitTime: 13, lwobs: 1.5, lengthOfStay: 2.9, census: 27 }
        }
      ]
    },
    {
      ...
      type: 'decrease',
      acuity: 'low',
      concurrentCensus: Math.round((150 * avgLengthOfStay) / 24),
      shifts: [
        {
          name: 'Day Shift',
          time: '7:00 AM - 7:00 PM',
          ...
          nurses: '-3',
          needsChange: true,
          reasoning: '22 concurrent patients (vs 29 baseline). Reduced to 3 physicians, 3 APPs, 7 RNs. Holiday low-acuity mix allows safe staffing reduction.',
        ...
        },
        {
          ...
          nurses: '-2',
          needsChange: true,
          reasoning: '18 concurrent patients (vs 23 baseline). Baseline physicians and APPs already cover the reduced load; only RNs can safely decrease by 2 while maintaining the 1:3 ratio.',
          baselineMetrics: { waitTime: 8, lwobs: 0.9, lengthOfStay: 2.3, census: 18 }
        }
      ]
    },
    {
      ...
      type: 'increase',
      acuity: 'high',
      concurrentCensus: Math.round((250 * avgLengthOfStay) / 24),
      shifts: [
        {
          name: 'Day Shift',
          time: '7:00 AM - 7:00 PM',
          ...
          nurses: '+2',
          needsChange: true,
          reasoning: '36 concurrent patients (vs 29 baseline). Weather-related trauma and respiratory emergencies require physician expertise. 1 additional physician, 2 RNs for intensive care needs. APPs maintain baseline.',
          baselineMetrics: { waitTime: 20, lwobs: 2.8, lengthOfStay: 3.6, census: 36 }
        },
        {
          name: 'Night Shift',
          time: '7:00 PM - 7:00 AM',
          ...
          nurses: '+2',
          needsChange: true,
          reasoning: '29 concurrent patients (vs 23 baseline) from weather-related trauma and respiratory cases carrying into the evening. 1 additional physician, 1 APP, and 2 RNs required to maintain safe ratios.',
          baselineMetrics: { waitTime: 15, lwobs: 1.8, lengthOfStay: 3.1, census: 29 }
        }
      ]
    },
    {
      ...
      type: 'increase',
      acuity: 'low',
      concurrentCensus: Math.round((228 * avgLengthOfStay) / 24),
      shifts: [
        {
          name: 'Day Shift',
          time: '7:00 AM - 7:00 PM',
          ...
          nurses: '+1',
          needsChange: true,
          reasoning: '33 concurrent patients (vs 29 baseline). Chronic condition exacerbations — routine cases handled efficiently by APP with nursing support.',
          baselineMetrics: { waitTime: 17, lwobs: 2.3, lengthOfStay: 3.3, census: 33 }
        },
        {
          name: 'Night Shift',
          time: '7:00 PM - 7:00 AM',
          ...
          nurses: '+1',
          needsChange: true,
          reasoning: '27 concurrent patients (vs 23 baseline) from month-end pattern continuing into the evening. 1 additional physician, 1 APP, and 1 RN needed to maintain standard ratios.',
          baselineMetrics: { waitTime: 12, lwobs: 1.4, lengthOfStay: 2.8, census: 27 }
        }
      ]
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-slate-100 p-6">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="bg-slate-800/60 rounded-lg p-3">
                <p className="text-xs text-slate-400 mb-1">Patients arriving</p>
                <p className="text-2xl font-bold text-cyan-400 mono"><span className="text-slate-400 font-light">~</span>{Math.round(dayShiftVolume)}</p>
              </div>
              <div className="bg-slate-800/60 rounded-lg p-3">
                <p className="text-xs text-slate-400 mb-1">Average census</p>
                <p className="text-2xl font-bold text-cyan-400 mono"><span className="text-slate-400 font-light">~</span>{dayShiftConcurrent}</p>
              </div>
            </div>
            <div className="bg-slate-800/60 rounded-lg p-3 mb-3">
              <p className="text-xs text-slate-400 mb-2">Recommended staffing</p>
              <div className="grid grid-cols-3 gap-2 text-center">
                <div>
                  <p className="text-xl font-bold text-emerald-400 mono">{dayShiftDoctors}</p>
                  <p className="text-xs text-slate-400">Physicians</p>
                </div>
                <div>
                  <p className="text-xl font-bold text-emerald-400 mono">{dayShiftAPPs}</p>
                  <p className="text-xs text-slate-400">APPs</p>
                </div>
                <div>
                  <p className="text-xl font-bold text-emerald-400 mono">{dayShiftRNs}</p>
                  <p className="text-xs text-slate-400">RNs</p>
                </div>
              </div>
            </div>
            <button
            ...
            >
              ...
              <span className={`transition-transform ${expandedDayOverview ? 'rotate-180' : ''}`}>▼</span>
            </button>
            {expandedDayOverview && (
              <div className="mt-3 pt-3 border-t border-slate-700/50">
                <p className="text-xs text-slate-400 leading-relaxed">
                  <span className="text-slate-300 font-semibold">~</span>{Math.round(dayShiftVolume)} patients arrive during the day shift, creating an average census of <span className="text-slate-300 font-semibold">~</span>{dayShiftConcurrent} (based on a {avgLengthOfStay}hr average LOS).
                  Industry ratios: 1 MD per <span className="text-amber-400 font-semibold">{doctorToConcurrentRatio}</span>, 1 APP per <span className="text-amber-400 font-semibold">{appToConcurrentRatio}</span>, 1 RN per <span className="text-amber-400 font-semibold">{nurseToConcurrentRatio}</span> patients.
                </p>
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="bg-slate-800/60 rounded-lg p-3">
                <p className="text-xs text-slate-400 mb-1">Patients arriving</p>
                <p className="text-2xl font-bold text-indigo-400 mono"><span className="text-slate-400 font-light">~</span>{Math.round(nightShiftVolume)}</p>
              </div>
              <div className="bg-slate-800/60 rounded-lg p-3">
                <p className="text-xs text-slate-400 mb-1">Average census</p>
                <p className="text-2xl font-bold text-indigo-400 mono"><span className="text-slate-400 font-light">~</span>{nightShiftConcurrent}</p>
              </div>
            </div>
            <div className="bg-slate-800/60 rounded-lg p-3 mb-3">
              <p className="text-xs text-slate-400 mb-2">Recommended staffing</p>
              <div className="grid grid-cols-3 gap-2 text-center">
                <div>
                  <p className="text-xl font-bold text-emerald-400 mono">{nightShiftDoctors}</p>
                  <p className="text-xs text-slate-400">Physicians</p>
                </div>
                <div>
                  <p className="text-xl font-bold text-emerald-400 mono">{nightShiftAPPs}</p>
                  <p className="text-xs text-slate-400">APPs</p>
                </div>
                <div>
                  <p className="text-xl font-bold text-emerald-400 mono">{nightShiftRNs}</p>
                  <p className="text-xs text-slate-400">RNs</p>
                </div>
              </div>
            </div>
            ...
            >
              <AlertCircle size={12} />
              <span className="font-semibold">Why these numbers?</span>
              <span className={`transition-transform ${expandedNightOverview ? 'rotate-180' : ''}`}>▼</span>
            </button>
            {expandedNightOverview && (
              <div className="mt-3 pt-3 border-t border-slate-700/50">
                <p className="text-xs text-slate-400 leading-relaxed">
                  <span className="text-slate-300 font-semibold">~</span>{Math.round(nightShiftVolume)} patients arrive during the night shift, creating an average census of <span className="text-slate-300 font-semibold">~</span>{nightShiftConcurrent} (based on a {avgLengthOfStay}hr average LOS).
                  Industry ratios: 1 MD per <span className="text-amber-400 font-semibold">{doctorToConcurrentRatio}</span>, 1 APP per <span className="text-amber-400 font-semibold">{appToConcurrentRatio}</span>, 1 RN per <span className="text-amber-400 font-semibold">{nurseToConcurrentRatio}</span> patients.
                </p>
              </div>
            )}
      ...
      <div className="card rounded-xl p-6 mb-8">
        ...
        {expandedRoiSettings && (
          <div className="grid md:grid-cols-5 gap-4 pt-4 border-t border-slate-700/50">
            <div>
            ...
            <div>
              <label className="block text-xs text-slate-400 mb-2">Physician Hourly Rate</label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">$</span>
                <input
                  type="number"
            ...
            <div>
              <label className="block text-xs text-slate-400 mb-2">APP Hourly Rate</label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">$</span>
                <input
                  type="number"
                  value={roiSettings.appHourlyRate}
                  onChange={(e) => setRoiSettings({...roiSettings, appHourlyRate: parseFloat(e.target.value) || 0})}
                  className="w-full pl-8 pr-3 py-2 bg-slate-900/70 border border-slate-700 rounded text-slate-200 font-mono text-sm focus:border-cyan-500 focus:outline-none"
                />
              </div>
            </div>
            ...
            <div>
              <label className="block text-xs text-slate-400 mb-2">RN Hourly Rate</label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">$</span>
                <input
                  type="number"
              ...
              <div className="grid grid-cols-2 gap-4 mb-5">
                <div className="bg-slate-900/50 rounded-lg p-4 border border-slate-700/30">
                  ...
                  <div className="border-t border-slate-700/50 pt-2 space-y-1">
                    <p className="text-xs text-slate-500 mono">
                      <span className="text-cyan-400/70">Day</span>
                      {'  '}<span className="text-slate-400 font-semibold">~</span><span className="text-slate-300 font-semibold">{Math.round(day.predicted * 0.6)}</span> arr · <span className="text-slate-400 font-semibold">~</span><span className="text-slate-300 font-semibold">{Math.round((day.predicted * 0.6 * avgLengthOfStay) / 12)}</span> census
                    </p>
                    <p className="text-xs text-slate-500 mono">
                      <span className="text-indigo-400/70">Night</span>
                      {'  '}<span className="text-slate-400 font-semibold">~</span><span className="text-slate-300 font-semibold">{Math.round(day.predicted * 0.4)}</span> arr · <span className="text-slate-400 font-semibold">~</span><span className="text-slate-300 font-semibold">{Math.round((day.predicted * 0.4 * avgLengthOfStay) / 12)}</span> census
                    </p>
                  </div>
                </div>
                ...
                <div className={`rounded-lg p-4 border flex flex-col justify-between ${
                  ...
                  <div className="border-t border-slate-700/50 mt-3 pt-3 space-y-1">
                    <p className="text-xs text-slate-500 mono">
                      <span className="text-slate-400">Monthly avg</span>{'  '}<span className="text-slate-300 font-semibold">{monthlyAverage}</span> patients/day
                    </p>
                    <p className="text-xs text-slate-500 mono">
                      <span className="text-slate-400">This day</span>{'  '}<span className="text-slate-300 font-semibold">{day.predicted}</span> patients
                    </p>
                  </div>
                </div>
              </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="grid grid-cols-3 gap-2 mb-3">
                          <div>
                            <p className="text-xs text-slate-400 mb-1">Physicians</p>
                          ...
                          <div>
                            ...
                            <p className={`text-xl font-bold mono ${
                              shift.needsChange && shift.apps !== '0'
                                ? (day.type === 'increase' ? 'text-red-400' : 'text-emerald-400')
                                : 'text-slate-500'
                            }`}>
                              {shift.apps}
                          ...
                          <div>
                            <p className="text-xs text-slate-400 mb-1">RNs</p>
                            <p className={`text-xl font-bold mono ${
                            ...
                            {expandedImpact[`${idx}-${shiftIdx}`] && (
                              <div className="mt-3 pt-3 border-t border-slate-700/50">
                                ...
                                <div className="grid grid-cols-3 gap-3 mb-4">
                                  {[
                                    { key: 'doctors', label: 'Physicians', max: 5 },
                                    { key: 'apps',    label: 'APPs',    max: 5 },
                                    { key: 'nurses',  label: 'RNs',     max: 10 },
                                  ].map(({ key, label, max }) => {
                                    const rec = shift[key];
                                    const val = staffingAdjustments[`${idx}-${shiftIdx}`]?.[key] ?? parseFloat(rec) ?? 0;
                                    const pos = (val + max) / (2 * max) * 100;
                                    const grad = val >= 0
                                      ? `linear-gradient(to right, #334155 0%, #334155 50%, #22d3ee 50%, #22d3ee ${pos}%, #334155 ${pos}%, #334155 100%)`
                                      : `linear-gradient(to right, #334155 0%, #334155 ${pos}%, #22d3ee ${pos}%, #22d3ee 50%, #334155 50%, #334155 100%)`;
                                    return (
                                      <div key={key}>
                                        <div className="flex items-center justify-between mb-2">
                                          ...
                                          <div className="text-right">
                                            ...
                                            <p className="text-xs text-slate-500">(Rec: {rec})</p>
                                          </div>
                                        </div>
                                        <input
                                          type="range"
                                          ...
                                          step="1"
                                          value={val}
                                          onChange={(e) => updateStaffingLevel(idx, shiftIdx, key, parseFloat(e.target.value) || 0)}
                                          className="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-cyan-500"
                                ...
                                {(() => {
                                  ...
                                  const metrics = calculateImpact(
                                    ...
                                    { doctors: shift.doctors, apps: shift.apps, nurses: shift.nurses, type: day.type },
                                    currentAdjustment
                                  );
                                ...
                                {(() => {
                                  const currentAdjustment = staffingAdjustments[`${idx}-${shiftIdx}`];
                                  const recDoctors = parseFloat(shift.doctors.replace(/[+-]/g, '')) || 0;
                                  const recAPPs = parseFloat(shift.apps.replace(/[+-]/g, '')) || 0;
                                  const recNurses = parseFloat(shift.nurses.replace(/[+-]/g, '')) || 0;
                                  const actualDoctors = currentAdjustment?.doctors ?? recDoctors;
                                  const actualAPPs = currentAdjustment?.apps ?? recAPPs;
                                  const actualNurses = currentAdjustment?.nurses ?? recNurses;
        ...
        {filteredAnomalyDays.length > 0 && (
          <div className="mt-6 pt-6 border-t border-slate-700/50">
            ...
            {(() => {
              ...
              let totalAPPShifts = 0;
              let totalNurseShifts = 0;
              let totalStaffingCostForSummary = 0;
                ...
                day.shifts.forEach((shift, shiftIdx) => {
                  if (shift.needsChange) {
                    ...
                    const actualMetrics = calculateImpact(
                      shift.baselineMetrics,
                      { doctors: shift.doctors, apps: shift.apps, nurses: shift.nurses, type: day.type },
                      { doctors: actualDoctors, apps: actualAPPs, nurses: actualNurses }
                    );
                    ...
                    if (!isFinite(improvement) || !isFinite(lwobsImprovement) || !isFinite(losImprovement)) {
                      console.error('⚠️ INFINITY DETECTED for shift:', shift.name, 'day:', day.date);
                    }
              ...
              const isNetIncrease = totalDoctorShifts > 0 || totalAPPShifts > 0 || totalNurseShifts > 0;
              const isNetDecrease = totalDoctorShifts < 0 || totalAPPShifts < 0 || totalNurseShifts < 0;

              return (
                <>
                  <div className="grid md:grid-cols-4 gap-4 mb-6">
                    <div className="bg-slate-900/50 rounded-lg p-5 border border-slate-700/30">
                      <p className="text-sm text-slate-400 mb-2">Total Physician Shifts</p>
                      <div className="flex items-center gap-2">
                        <p className={`text-3xl font-bold mono ${
                    ...
                    <div className="bg-slate-900/50 rounded-lg p-5 border border-slate-700/30">
                      <p className="text-sm text-slate-400 mb-2">Total APP Shifts</p>
                      <div className="flex items-center gap-2">
                        <p className={`text-3xl font-bold mono ${
                          totalAPPShifts > 0 ? 'text-red-400' : totalAPPShifts < 0 ? 'text-emerald-400' : 'text-slate-400'
                        }`}>
                          {totalAPPShifts > 0 ? '+' : ''}{totalAPPShifts}
                        </p>
                        <span className="text-sm text-slate-400">shifts</span>
                      </div>
                      <p className="text-xs text-slate-500 mt-2">
                        {totalAPPShifts > 0 ? 'Based on your adjustments' : totalAPPShifts < 0 ? 'Shifts can be reduced' : 'No net change'}
                      </p>
                    </div>

                    <div className="bg-slate-900/50 rounded-lg p-5 border border-slate-700/30">
                      <p className="text-sm text-slate-400 mb-2">Total RN Shifts</p>
                      <div className="flex items-center gap-2">
                        <p className={`text-3xl font-bold mono ${
